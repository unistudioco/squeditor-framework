    ...
    var Component = {
      ...
      methods: {
        ...
        updateActiveClasses(currentIndex = this.index) {
          ...
          for (const slide of this.slides) {
            util.toggleClass(slide, activeClasses, util.includes(actives, slide));
          }
        },
        getValidIndex(index = this.index, prevIndex = this.prevIndex) {
          index = this.getIndex(index, prevIndex);
    ...
    function preventBackgroundScroll(el) {
      const off = util.on(
        ...
        (e) => {
          ...
          util.once(el, "scroll touchend touchcancel", offMove, { capture: true });
        },
        { passive: true }
      );
      if (prevented) {
      ...
      const props = {
        overflowY: CSS.supports("overflow", "clip") ? "clip" : "hidden",
        touchAction: "none",
        scrollbarGutter: util.width(window) - scrollingElement.clientWidth ? "stable" : ""
      };
